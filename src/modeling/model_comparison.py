...
if __name__ == "__main__":
    ...
    _OUT = _ROOT / "models" / "saved_models"
    ...
    X_train, X_test, y_train, y_test, feat_names = load_and_split(
        str(_DATA / "engineered_features.csv"), str(_DATA / "target_severity.csv")
    )
    ...
    X_full = pd.read_csv(_DATA / "engineered_features.csv")
    y_full = pd.read_csv(_DATA / "target_severity.csv").squeeze()
    cv_results = {}
    ...
    plot_comparison(
        ...
        X_test, y_test,
        output=str(_OUT / "comparison_figure.png")
    )
    plot_roc_curves(models["Random Forest (tuned)"], models["XGBoost (tuned)"],
                    X_test, y_test,
                    output=str(_OUT / "roc_curves.png"))
    ...
    fi_df.to_csv(_OUT / "feature_importance_comparison.csv", index=False)

    # 7. Save best model
    pickle.dump(models["XGBoost (tuned)"],   open(_OUT / "xgb_model.pkl", "wb"))
    pickle.dump(models["Random Forest (tuned)"], open(_OUT / "rf_model.pkl", "wb"))
    ...
    write_report(results, cv_results, rf_params_desc, xgb_params_desc,
                 output=str(_OUT / "comparison_report.txt"))
