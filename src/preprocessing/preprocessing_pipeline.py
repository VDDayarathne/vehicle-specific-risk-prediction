...
OSM_ROADS_CSV  = str(_ROOT / "data" / "raw" / "kadugannawa_road_segments.csv")
TELEMETRY_CSV  = str(_ROOT / "data" / "raw" / "telemetry" / "telemetry_raw.csv")
OUTPUT_DIR     = str(_ROOT / "data" / "processed")
MODEL_OUT_DIR  = str(_ROOT / "models" / "pipeline_objects")
...
def load_datasets(accidents_path: str, weather_path: str,
                  ...
                  telemetry_path: str | None = None) -> tuple:
    """Load source datasets. Returns (accidents_df, weather_df, osm_df|None, telemetry_df|None)."""
    print("[1/9] Loading datasets...")

    accidents = pd.read_csv(accidents_path)
    accidents = normalize_accident_frame(accidents)
    print(f"      Accidents : {accidents.shape[0]} rows x {accidents.shape[1]} cols")
    ...
    if osm_path and os.path.exists(osm_path):
        ...
        print(f"      OSM roads : {osm.shape[0]} rows x {osm.shape[1]} cols")
    else:
    ...
    if telemetry_path and os.path.exists(telemetry_path):
        ...
        print(f"      Telemetry : {telemetry.shape[0]} rows x {telemetry.shape[1]} cols")
    else:
        print("      Telemetry : not found — skipping")

    return accidents, weather, osm, telemetry
...
def merge_datasets(accidents: pd.DataFrame, weather: pd.DataFrame,
                   osm: pd.DataFrame | None,
                   telemetry: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    ...
    # Telemetry merge — optional, light-weight join for vehicle/device context
    if telemetry is not None and len(telemetry) > 0:
        telemetry = telemetry.copy()
        if "timestamp" in telemetry.columns:
            telemetry["date"] = pd.to_datetime(telemetry["timestamp"], errors="coerce").dt.strftime("%Y-%m-%d")
            telemetry["hour"] = pd.to_datetime(telemetry["timestamp"], errors="coerce").dt.hour

        telemetry_agg = telemetry.copy()
        if {"date", "hour"}.issubset(telemetry_agg.columns):
            group_cols = [c for c in ["date", "hour", "vehicle_type"] if c in telemetry_agg.columns]
            if group_cols:
                telemetry_agg = telemetry_agg.groupby(group_cols, as_index=False).agg({
                    "speed_kmh": "mean" if "speed_kmh" in telemetry_agg.columns else "first",
                    "heading_deg": "mean" if "heading_deg" in telemetry_agg.columns else "first",
                    "accel_m_s2": "mean" if "accel_m_s2" in telemetry_agg.columns else "first",
                    "rpm": "mean" if "rpm" in telemetry_agg.columns else "first",
                    "latitude": "mean" if "latitude" in telemetry_agg.columns else "first",
                    "longitude": "mean" if "longitude" in telemetry_agg.columns else "first",
                })
                telemetry_agg = telemetry_agg.rename(columns={
                    "speed_kmh": "telemetry_speed_kmh",
                    "heading_deg": "telemetry_heading_deg",
                    "accel_m_s2": "telemetry_accel_m_s2",
                    "rpm": "telemetry_rpm",
                    "latitude": "telemetry_latitude",
                    "longitude": "telemetry_longitude",
                })

                join_cols = [c for c in ["date", "hour", "vehicle_type"] if c in df.columns and c in telemetry_agg.columns]
                if join_cols:
                    df = df.merge(telemetry_agg, on=join_cols, how="left")
                    telemetry_cov = df.get("telemetry_speed_kmh", pd.Series(dtype=float)).notna().mean() if "telemetry_speed_kmh" in df.columns else 0
                    print(f"      Telemetry merge coverage : {telemetry_cov:.1%}")
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Telemetry features ────────────────────────────────────────────────────
    if "telemetry_speed_kmh" in df.columns:
        df["telemetry_speed_ratio"] = (
            df["telemetry_speed_kmh"] / df["speed_limit_kmh"].replace(0, 50)
        ).round(3)
    if "telemetry_accel_m_s2" in df.columns:
        df["telemetry_brake_index"] = (df["telemetry_accel_m_s2"] < -1.5).astype(int)
    if {"telemetry_latitude", "telemetry_longitude"}.issubset(df.columns):
        df["telemetry_gps_valid"] = df[["telemetry_latitude", "telemetry_longitude"]].notna().all(axis=1).astype(int)
    ...
    new_feats = [
        ...
        "rain_intensity_num", "visibility_cls_enc", "telemetry_speed_ratio",
        "telemetry_brake_index", "telemetry_gps_valid",
    ]
...
def save_outputs(X_train, X_test, y_train, y_test,
    ...
    with open(f"{MODEL_OUT_DIR}/scaler.pkl",  "wb") as f: pickle.dump(scaler,  f)
    with open(f"{MODEL_OUT_DIR}/knn_imp.pkl", "wb") as f: pickle.dump(knn_imp, f)
    with open(f"{MODEL_OUT_DIR}/med_imp.pkl", "wb") as f: pickle.dump(med_imp, f)
    ...
    Dataset sizes
    -------------
    Training (post-SMOTE) : {X_train.shape[0]} rows x {X_train.shape[1]} features
    Test (held-out)       : {X_test.shape[0]} rows x {X_test.shape[1]} features
...
def preprocess_single_request(request: dict, feature_names: list,
    ...
    direct = [
        ...
        "gradient_pct", "traffic_density", "vehicle_type", "latitude",
        "longitude", "speed_kmh", "heading_deg", "accel_m_s2", "rpm",
    ]
    for col in direct:
    ...
    telemetry_speed = request.get("speed_kmh", request.get("estimated_speed_kmh", 50))
    row["telemetry_speed_kmh"] = telemetry_speed
    row["telemetry_speed_ratio"] = telemetry_speed / max(1, request.get("speed_limit_kmh", 50))
    row["telemetry_brake_index"] = int(request.get("accel_m_s2", 0) < -1.5)
    row["telemetry_gps_valid"] = int(request.get("latitude") is not None and request.get("longitude") is not None)
...
def run_pipeline():
    ...
    accidents, weather, osm, telemetry = load_datasets(ACCIDENTS_CSV, WEATHER_XLSX, OSM_ROADS_CSV, TELEMETRY_CSV)
    accidents, weather      = clean_dates(accidents, weather)
    weather                 = clean_weather(weather)
    df                      = merge_datasets(accidents, weather, osm, telemetry)
    ...
    print("\n" + "="*60)
    print("  [OK] Pipeline complete. Ready for model training.")
