...
def vehicle_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["vehicle_risk_weight"] = df["vehicle_type"].map(RISK).fillna(2)
...
def road_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    The slope x curvature interaction captures this compounding effect.
    ...
    # Slope x curvature interaction (continuous product)
...
def weather_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    The rain x curvature interaction (MI=0.119) captures that rain on a
...
def location_risk_encoding(df: pd.DataFrame, y: pd.Series,
    ...
    Why not OHE? 10 locations x 500 rows = very sparse. Mean encoding
...
def build_features(csv_path: str,
    ...
    Returns:
        X (pd.DataFrame): engineered feature matrix (500 rows x ~50 cols)
    ...
    print(f"\n    Output: {X.shape[0]} rows x {X.shape[1]} features")
...
if __name__ == "__main__":
    ...
    X, y, registry = build_features(str(_PROCESSED / "merged_accidents_weather.csv"))

    # Save
    X.to_csv(_PROCESSED / "engineered_features.csv", index=False)
    y.to_csv(_PROCESSED / "target_severity.csv", index=False, header=True)
    registry.to_csv(_PROCESSED / "feature_registry.csv", index=False)

    print(f"\nSaved:")
    print(f"  data/processed/engineered_features.csv   ({X.shape[0]} rows x {X.shape[1]} cols)")
    print(f"  data/processed/target_severity.csv       ({len(y)} labels)")
    print(f"  data/processed/feature_registry.csv      ({len(registry)} features documented)")
