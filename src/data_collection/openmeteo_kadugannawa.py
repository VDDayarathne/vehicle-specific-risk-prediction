...
if __name__ == "__main__":
    ...
    import pathlib
    _root = pathlib.Path(__file__).resolve().parents[2]
    forecast_df.to_csv(_root / "data" / "external" / "kadugannawa_forecast_7d.csv", index=False)
    print(f"    Saved: data/external/kadugannawa_forecast_7d.csv ({len(forecast_df)} rows)")
    ...
    hist_daily.to_csv(_root / "data" / "external" / "kadugannawa_historical_daily.csv", index=False)
    print(f"    Saved: data/external/kadugannawa_historical_daily.csv ({len(hist_daily)} rows)")
    print("    Rain stats:")
    ...
    merged.to_csv(_root / "data" / "processed" / "accidents_with_realweather.csv", index=False)
    print(f"    Final dataset: {merged.shape[0]} rows x {merged.shape[1]} cols")
