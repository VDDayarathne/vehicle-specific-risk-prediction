...
Output:
  merged_accidents_weather.csv    (500 rows × 45 cols, 0 NaN in wx cols)
...
OUTPUT_CSV    = "merged_accidents_weather.csv"
...
def validate_and_save(df: pd.DataFrame, output_path: str) -> pd.DataFrame:
    ...
    if null_counts.sum() > 0:
        ...
        print(f"✓ All {len(wx_cols)} wx columns complete (0 NaN)")
...
def merge_pipeline(acc_path=ACCIDENTS_CSV, wx_path=WEATHER_XLSX, out=OUTPUT_CSV):
    print("=" * 56)
    print("  Weather × Accident Merge — Kadugannawa Pass")
    print("=" * 56)
    ...
    print("\n" + "=" * 56)
    print("  ✓ Merge complete. Next: preprocessing_pipeline.py")
    print("=" * 56)
    return merged
